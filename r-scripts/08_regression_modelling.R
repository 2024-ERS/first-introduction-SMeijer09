...
orchdat<-readr::read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vT6bdROUwchBBIreCQQdBenu35D5Heo8bl3UgES9wrpBwax_GUM1bKSo_QXfmLq8Ml9-crCI7MmW2XH/pub?gid=615066255&single=true&output=csv") |> dplyr::filter(year %in% c(2018,2019,2021:2024), Species_ID=="Orchestia_gammarellus", replicate<=3) |> dplyr::group_by(year,TransectPoint_ID) |> dplyr::summarise(CountSum=sum(Count,na.rm=T))
view(orchdat)
...
orchdat3 |>
  ggplot2::ggplot(aes(x=factor(TransectPoint_ID),y=elevation_m,group=year)) +
  geom_bar(stat="identity") +
  facet_grid(year~.)
  
# plot Orchestia (y) versus elevation_m (x) in ggplot as a scatterplot, with each year as a different color
p1<- orchdat3  |> 
  ggplot2::ggplot(mapping=aes(x=elevation_m,y=CountSum, color=year)) +
  geom_point(size=3) 
p1
# calculate the optimal preferred elevation by Orchestia for each year using weighted.mean function
orchdat3 |> 
  dplyr::group_by(year) |>
  dplyr::summarise(wa_elevation=weighted.mean(elevation_m,w=CountSum,na.rm=T))
view(orchdat3)
...
p1 + geom_line(data=orchdat3, aes(y=pred7,color=factor(year)),linewidth=1.2)
anova(m7,test="Chisq")
...
p1 + geom_line(data=orchdat3, aes(y=pred8,color=factor(year)),linewidth=1.2)
...
#  geom_point(size=3) +
  geom_smooth(method="glm",formula="y~x+I(x^2)", se=F,
